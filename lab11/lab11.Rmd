...
## power, usually the benchmark is 80%. the power of design, to what extent the p-value of estimand is over 95%.
...
declaration_18.4a <-
    declare_model(
        ...
        U = rnorm(N, sd = 1),
        potential_outcomes(Y ~ 0.2 * Z + gender + U)
    ) +
    declare_assignment(
        Z = block_ra(blocks = gender),
        # Z = complete_ra(N = N, prob = 0.5),
        # Z = simple_racomplete_ra(N = N, prob = 0.5),
    ) +
    declare_measurement(Y = reveal_outcomes(Y ~ Z)) +
    declare_estimator(
        Y ~ Z,
...
dd <- diagnose_design(declaration_18.4a)
dd$diagnosands_df
dd$simulations_df %>% head()
```
...
Compare block randomization to complete or simple randomization while varying 1 or more of the following dimensions:
- Sample size
- Treatment effect
- Magnitude of gender effect on the outcome
- Adding in other covariates 
