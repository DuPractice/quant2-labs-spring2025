...
pacman::p_load(fixest, wildrwolf, broom, tidyverse)
p.values <- get_pvalues(models)
```

# 3

Write a simulation that repeatedly draws new data from the DGP and calculates the p.values with multiple strategies. Then, based on the simulation results, calculate the following three quantities of interest:
...
2. FWER - the family-wise error rate. This is related to the FDR, but is slightly different. If we consider one set of fixest models as a single "family," the FWER is the probability that we have at least one false discovery. 
